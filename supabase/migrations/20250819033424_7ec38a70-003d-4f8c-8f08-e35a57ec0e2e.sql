...
INSERT INTO public.projects (title, description, category, price_inr, status, seller_id, tech_stack, features) 
SELECT 
  'Sample E-commerce Dashboard',
  'A complete admin dashboard for e-commerce with inventory management and analytics',
  'dashboard',
  25000,
  'approved',
...
SELECT 
  ...
  'Responsive portfolio website with contact forms and project showcase',
  'portfolio',
  12000,
  'approved',
  (SELECT user_id FROM public.profiles LIMIT 1),
  '["React", "Tailwind CSS"]'::jsonb,
  '["Responsive Design", "Contact Form", "Portfolio Showcase"]'
WHERE EXISTS (SELECT 1 FROM public.profiles LIMIT 1);

INSERT INTO public.projects (title, description, category, price_inr, status, seller_id, tech_stack, features) 
SELECT 
  'SaaS Landing Page',
  'High-converting landing page with pricing tables and testimonials',
  'landing page',
  8000,
  'approved',
  (SELECT user_id FROM public.profiles LIMIT 1),
  '["Next.js", "Tailwind CSS"]'::jsonb,
  '["Responsive Design", "Pricing Tables", "Testimonials"]'
WHERE EXISTS (SELECT 1 FROM public.profiles LIMIT 1);